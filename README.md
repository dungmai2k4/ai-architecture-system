...
## Architecture Design Platform Upgrade

The backend now extends the original floorplan response with an `ArchitecturalDesignPackage` generated by deterministic architecture modules before any future AI enhancement. The package adds Vietnamese residential typology selection, reusable spatial patterns, climate analysis, courtyard planning, roof planning, building sections, facade composition, exterior style guidance, landscape planning, and render prompts for exterior/courtyard/facade/section views.

New deterministic modules live under `backend/src/main/java/com/architectai/design/architecture/` and use extendable JSON knowledge datasets in `backend/src/main/resources/knowledge/` for Vietnamese typologies, tropical design rules, facade patterns, courtyard patterns, roof patterns, and spatial patterns.

The enriched API response includes:

```json
{
  "architecturalDesignPackage": {
    "siteInfo": {},
    "typology": {},
    "climateAnalysis": {},
    "spatialPatterns": [],
